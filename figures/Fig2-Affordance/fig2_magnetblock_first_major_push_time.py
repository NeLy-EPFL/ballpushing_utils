...
def main(test_mode=False):
    ...
    # Raw seconds — used for stats only
    control_raw = df[df["Magnet"] == "n"][METRIC].values
    test_raw = df[df["Magnet"] == "y"][METRIC].values

    # Convert to minutes + offset by 60 for display (y-axis starts at 60 min)
    ...
    # --- Permutation test (median difference, on raw seconds — matches legacy script) ---
    ...
    print(f"Median diff: {obs_diff:.3f} s  |  p = {p_value}  |  {sig_symbol}")
    ...
    out_pdf = OUTPUT_DIR / "first_major_event_time_magnetblock.pdf"
    out_png = OUTPUT_DIR / "first_major_event_time_magnetblock.png"
    ...
    stats = {
        ...
        "control_n": n1,
        "test_n": n2,
        "control_median_min": np.median(control),
        "test_median_min": np.median(test),
        "median_diff_min": np.median(test) - np.median(control),
        "p_value": p_value,
        "significance": sig_symbol,
        "n_permutations": N_PERMUTATIONS,
    }
    stats_df = pd.DataFrame([stats])
    stats_csv = OUTPUT_DIR / "first_major_event_time_magnetblock_stats.csv"
