#!/usr/bin/env python3
"""
Script to create a scatter plot of training vs test ball distance_ratio for Pretraining == "y" flies.
X-axis: distance_ratio for ball_condition "training"
Y-axis: distance_ratio for ball_condition "test"
"""
...
def main():
    ...
    # Dataset path
    dataset_path = (
        "/mnt/upramdya_data/MD/F1_Tracks/Datasets/251001_12_summary_F1_New_Data/summary/pooled_summary.feather"
    )
    ...
    # Clean the data
    df_clean = df[[distance_ratio_col, pretraining_col, ball_condition_col, fly_id_col]].dropna()
    print(f"Data shape after removing NaNs: {df_clean.shape}")

    # Debug: Show some sample data to verify the distance ratio values are different
    sample_data = df_clean[[distance_ratio_col, ball_condition_col, fly_id_col]].head(10)
    ...
    # Pivot the data to get training and test distance ratios for each fly
    pivot_data = pretrained_flies.pivot_table(
        index=fly_id_col,
        columns=ball_condition_col,
        values=distance_ratio_col,
    ...
    # Formatting
    plt.xlabel(f"Distance Ratio - Training Ball", fontsize=12)
    plt.ylabel(f"Distance Ratio - Test Ball", fontsize=12)
    plt.title("Training vs Test Ball Distance Ratio\n(Pretrained Flies Only)", fontsize=14, fontweight="bold")
    ...
    # Save the plot
    output_path = Path(__file__).parent / "training_vs_test_distance_ratio_scatter.png"
