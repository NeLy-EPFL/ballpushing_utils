#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for fly_distance_moved metric.
Analyzes fly_distance_moved (activity measure) by Pretraining condition.
"""
...
def main():
    ...
    fly_distance_col = None
    pretraining_col = None
    ball_condition_col = None

    # Look for fly_distance_moved column
    if "fly_distance_moved" in df.columns:
        fly_distance_col = "fly_distance_moved"
    else:
        # Look for similar columns
        for col in df.columns:
            if "fly" in col.lower() and "distance" in col.lower() and ("move" in col.lower() or "moved" in col.lower()):
                fly_distance_col = col
                break
    ...
    df_clean = df[[fly_distance_col, pretraining_col, ball_condition_col]]
    ...
    sample_data = df_clean[[fly_distance_col, ball_condition_col, pretraining_col]].head(10)
    ...
    print(f"  {fly_distance_col}: {test_ball_data[fly_distance_col].isna().sum()} missing out of {len(test_ball_data)}")

    # Show actual values for debugging
    print(f"\nSample of actual values:")
    print(f"{fly_distance_col} (first 10 non-null values):")
    distance_sample = test_ball_data[fly_distance_col].dropna().head(10)
    ...
    valid_data = test_ball_data[test_ball_data[fly_distance_col].notna()]
    if not valid_data.empty:
        valid_dist = valid_data[pretraining_col].value_counts()
        print(valid_dist)

        # Show basic stats by condition
        print(f"\nBasic statistics by {pretraining_col}:")
        for condition in valid_data[pretraining_col].unique():
            subset = valid_data[valid_data[pretraining_col] == condition][fly_distance_col]
            print(f"  {condition}: Mean = {subset.mean():.3f}, Std = {subset.std():.3f}, n = {len(subset)}")
    else:
    ...
    # Plot fly_distance_moved by Pretraining condition (only for non-missing data)
    distance_data = test_ball_data.dropna(subset=[fly_distance_col])
    if not distance_data.empty:
        create_boxplot_with_scatter(
            data=distance_data,
            x_col=pretraining_col,
            y_col=fly_distance_col,
            title="Fly Distance Moved (Activity) by Pretraining Condition",
            ax=ax,
            color_palette=pretraining_palette,
        )
    else:
        ax.text(0.5, 0.5, "No valid data for fly_distance_moved", ha="center", va="center", transform=ax.transAxes)
        ax.set_title("Fly Distance Moved (Activity) by Pretraining Condition")
    ...
    if pretraining_col and fly_distance_col:
        print(f"\nFly Distance Moved by {pretraining_col}:")
        distance_data = test_ball_data.dropna(subset=[fly_distance_col])
        if not distance_data.empty:
            distance_summary = (
                distance_data.groupby(pretraining_col)[fly_distance_col]
            ...
            for condition in distance_data[pretraining_col].unique():
                subset = distance_data[distance_data[pretraining_col] == condition][fly_distance_col]
                condition_mean = subset.mean()
                relative_activity = condition_mean / overall_mean if overall_mean > 0 else 0
                print(f"  {condition}: {relative_activity:.2f}x overall activity level")
        else:
