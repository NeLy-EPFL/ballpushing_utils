#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for pulling_ratio metric for TNT LC10-2 dataset.
Analyzes pulling_ratio by Genotype and Pretraining condition.
"""
...
def main():
    ...
    # Check value range for ratio data interpretation
    ...
    if not ratio_values.empty:
        ...
        print(f"Median: {ratio_values.median():.3f}")

        # Ratio distribution
        print(f"Pulling ratio distribution:")
        zero_ratio = (ratio_values == 0).sum()
        low_ratio = ((ratio_values > 0) & (ratio_values <= 0.1)).sum()
        med_ratio = ((ratio_values > 0.1) & (ratio_values <= 0.5)).sum()
        high_ratio = (ratio_values > 0.5).sum()
        total = len(ratio_values)

        print(f"  Zero pulling: {zero_ratio}/{total} ({100*zero_ratio/total:.1f}%)")
        print(f"  Low pulling (0-10%): {low_ratio}/{total} ({100*low_ratio/total:.1f}%)")
        print(f"  Medium pulling (10-50%): {med_ratio}/{total} ({100*med_ratio/total:.1f}%)")
        print(f"  High pulling (>50%): {high_ratio}/{total} ({100*high_ratio/total:.1f}%)")
    ...
    create_boxplot_with_scatter_combined(
        ...
        title="Pulling Ratio by Genotype + Pretraining (TNT LC10-2)",
        ax=ax,
    ...
    if pulling_ratio_col:
        ...
        if not ratio_data.empty:
            ...
            for group in ratio_data["combined_group"].unique():
                ...
                flies_with_pulling = (subset > 0).sum()
                flies_no_pulling = (subset == 0).sum()
                mean_among_pullers = subset[subset > 0].mean() if flies_with_pulling > 0 else 0

                print(f"  {group}:")
                print(
                    f"    Flies with pulling: {flies_with_pulling}/{total_flies} ({100*flies_with_pulling/total_flies:.1f}%)"
                )
                print(
                    f"    Flies with no pulling: {flies_no_pulling}/{total_flies} ({100*flies_no_pulling/total_flies:.1f}%)"
                )
                if flies_with_pulling > 0:
                    print(f"    Mean ratio among pullers: {mean_among_pullers:.3f}")
        else:
