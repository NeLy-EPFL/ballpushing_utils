...
# Dataset path
DATASET_PATH = '/mnt/upramdya_data/MD/F1_Tracks/Datasets/260104_12_F1_coordinates_F1_New_Data/summary/pooled_summary.feather'
...
def plot_pc_boxplot(pca_df, pc_name, grouping_col, selected_values, grouping_type, output_dir):
    ...
    if mw_results:
        ...
        for i, result in enumerate(mw_results):
            ...
            ax.plot([x1, x1, x2, x2],
                   ...
                   'k-', linewidth=1.5)
            ax.text((x1 + x2) / 2, y_bracket + y_range*0.01, sig_marker,
    ...
    if mw_results:
        ...
        for result in mw_results:
            print(f"    {result['group1']} vs {result['group2']}: p = {result['p_corrected']:.4f} "
...
def plot_3d_pca(pca_df, grouping_col, selected_values, grouping_type, output_dir):
    ...
    for group_val in group_values:
        ...
        ax.scatter(subset['PC1'], subset['PC2'], subset['PC3'],
                  s=SCATTER_SIZE, alpha=SCATTER_ALPHA,
                  c=[color], edgecolors='black', linewidths=0.5,
                  marker='o', label=label, depthshade=True)
...
def main():
    ...
    X_scaled, scaler, valid_metrics, metadata_df = prepare_pca_data(
        df, PCA_METRICS, grouping_col=f1_condition_col
    )
    ...
    for analysis in analyses:
        ...
        plot_pca_scatter(pca_df, 'PC1', 'PC2',
                        analysis['grouping_col'],
                        analysis['selected_values'],
                        analysis['grouping_type'],
                        analysis_dir)
        plot_pca_scatter(pca_df, 'PC1', 'PC3',
                        analysis['grouping_col'],
                        analysis['selected_values'],
                        analysis['grouping_type'],
                        analysis_dir,
