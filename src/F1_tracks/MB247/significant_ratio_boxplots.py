#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for significant_ratio metric for TNT MB247 dataset.
Analyzes significant_ratio by Genotype and Pretraining condition.
"""
...
def main():
    ...
    significant_ratio_col = "significant_ratio" if "significant_ratio" in df.columns else None
    ...
    df_clean = df[[significant_ratio_col, pretraining_col, genotype_col, ball_condition_col]]
    ...
    mask = pd.isna(plot_data[significant_ratio_col]) | np.isinf(plot_data[significant_ratio_col])
    ...
    # Plot: significant_ratio by Genotype + Pretraining
    create_boxplot_with_scatter_combined(
        ...
        title="Significant Ratio by Genotype + Pretraining (TNT MB247)",
        ax=ax,
    ...
    if significant_ratio_col:
        print(f"\nSignificant Ratio by Genotype + Pretraining:")
        ratio_data = plot_data_combined.dropna(subset=[significant_ratio_col])
        if not ratio_data.empty:
            ratio_summary = (
                ratio_data.groupby("combined_group")[significant_ratio_col]
