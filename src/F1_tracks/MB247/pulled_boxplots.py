#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for pulled metric for TNT MB247 dataset.
Analyzes pulled (pull event count) by Genotype and Pretraining condition.
"""
...
def main():
    ...
    # Check value range for pulled count interpretation
    print(f"\nValue range for {pulled_col}:")
    pulled_values = test_ball_data[pulled_col].dropna()
    if not pulled_values.empty:
        print(f"Min: {pulled_values.min()}, Max: {pulled_values.max()}")
        print(f"Mean: {pulled_values.mean():.3f}, Std: {pulled_values.std():.3f}")
        print(f"Median: {pulled_values.median():.3f}")

        # Count distribution for discrete data
        value_counts = pulled_values.value_counts().sort_index()
        print(f"Value distribution (count: frequency):")
    ...
    create_boxplot_with_scatter_combined(
        ...
        y_col=pulled_col,
        title="Pull Event Count by Genotype + Pretraining (TNT MB247)",
        ax=ax,
    ...
    if pulled_col:
        print(f"\nPull Event Count by Genotype + Pretraining:")
        ...
        if not pulled_data.empty:
            ...
            # Additional analysis: pull behavior interpretation
            print(f"\nPull Behavior Interpretation:")
            overall_mean = pulled_data[pulled_col].mean()
            print(f"Overall mean pull events: {overall_mean:.3f}")

            # Calculate percentage of flies with pull events
            for group in pulled_data["combined_group"].unique():
                ...
                pull_percentage = (subset > 0).mean() * 100
                zero_pulls = (subset == 0).sum()
                total_flies = len(subset)
                print(
                    f"  {group}: {group_mean:.2f} avg pulls, {pull_percentage:.1f}% showed pulling ({zero_pulls}/{total_flies} showed no pulls)"
                )
        else:
