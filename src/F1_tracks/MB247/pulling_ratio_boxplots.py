#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for pulling_ratio metric for TNT MB247 dataset.
Analyzes pulling_ratio (ratio-based behavioral measure) by Genotype and Pretraining condition.
"""
...
def main():
    ...
    # Dataset path - MB247 TNT dataset
    dataset_path = "/mnt/upramdya_data/MD/F1_Tracks/Datasets/251015_17_F1_coordinates_F1_TNT_MB247_Data/summary/pooled_summary.feather"
    ...
    pulling_ratio_col = "pulling_ratio" if "pulling_ratio" in df.columns else None
    ...
    df_clean = df[[pulling_ratio_col, pretraining_col, genotype_col, ball_condition_col]]
    ...
    # Check value range for pulling ratio interpretation
    print(f"\nValue range for {pulling_ratio_col}:")
    ratio_values = test_ball_data[pulling_ratio_col].dropna()
    if not ratio_values.empty:
        print(f"Min: {ratio_values.min():.4f}, Max: {ratio_values.max():.4f}")
        print(f"Mean: {ratio_values.mean():.4f}, Std: {ratio_values.std():.4f}")
        print(f"Median: {ratio_values.median():.4f}")

        # Check for ratio characteristics
        valid_ratios = ratio_values[(ratio_values >= 0) & (ratio_values <= 1)]
        print(
            f"Valid ratios (0-1): {len(valid_ratios)}/{len(ratio_values)} ({len(valid_ratios)/len(ratio_values)*100:.1f}%)"
        )

        # Distribution analysis
        zero_ratios = (ratio_values == 0).sum()
        one_ratios = (ratio_values == 1).sum()
        print(f"Zero ratios: {zero_ratios} ({zero_ratios/len(ratio_values)*100:.1f}%)")
        print(f"Perfect ratios (1.0): {one_ratios} ({one_ratios/len(ratio_values)*100:.1f}%)")

    # Prepare data for plotting
    plot_data = test_ball_data.copy()
    mask = pd.isna(plot_data[pulling_ratio_col]) | np.isinf(plot_data[pulling_ratio_col])
    ...
    # Plot: pulling_ratio by Genotype + Pretraining
    create_boxplot_with_scatter_combined(
        data=plot_data_filtered,
        pretraining_col=pretraining_col,
        genotype_col=genotype_col,
        y_col=pulling_ratio_col,
        title="Pulling Ratio by Genotype + Pretraining (TNT MB247)",
        ax=ax,
    ...
    # Save the plot
    output_path = Path(__file__).parent / "pulling_ratio_boxplots.png"
    ...
    if pulling_ratio_col:
        print(f"\nPulling Ratio by Genotype + Pretraining:")
        ratio_data = plot_data_combined.dropna(subset=[pulling_ratio_col])
        if not ratio_data.empty:
            ratio_summary = (
                ratio_data.groupby("combined_group")[pulling_ratio_col]
                .agg(["count", "mean", "std", "median", "min", "max"])
                .round(4)
            )
            print(ratio_summary)

            # Additional analysis: pulling efficiency interpretation
            print(f"\nPulling Efficiency Interpretation:")
            overall_mean = ratio_data[pulling_ratio_col].mean()
            print(f"Overall mean pulling ratio: {overall_mean:.4f}")

            # Analyze distribution characteristics by group
            for group in ratio_data["combined_group"].unique():
                subset = ratio_data[ratio_data["combined_group"] == group][pulling_ratio_col]
                group_mean = subset.mean()
                zero_ratio_pct = (subset == 0).mean() * 100
                high_ratio_pct = (subset > 0.5).mean() * 100
                perfect_ratio_pct = (subset == 1.0).mean() * 100
                print(f"  {group}: {group_mean:.4f} avg ratio")
                print(
                    f"    {zero_ratio_pct:.1f}% no pulling, {high_ratio_pct:.1f}% high pulling (>0.5), {perfect_ratio_pct:.1f}% perfect pulling (1.0)"
                )
        else:
            print("No valid data for pulling_ratio")
