#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for distance moved metrics for MB247 dataset.
1. distance_moved by Pretraining condition
2. distance_moved by Genotype
"""
...
def main():
    ...
    distance_moved_col = None
    pretraining_col = None
    genotype_col = None
    ball_condition_col = None

    # Look for distance_moved column
    if "distance_moved" in df.columns:
        distance_moved_col = "distance_moved"
    else:
        # Look for similar columns
        for col in df.columns:
            if "distance" in col.lower() and "moved" in col.lower():
                distance_moved_col = col
                break
    ...
    required_columns = {
        "distance_moved": distance_moved_col,
    ...
    print(f"  Distance moved: {distance_moved_col}")
    ...
    df_clean = df[[distance_moved_col, pretraining_col, genotype_col, ball_condition_col]]
    ...
    sample_data = df_clean[[distance_moved_col, ball_condition_col, pretraining_col, genotype_col]].head(10)
    ...
    # Check for missing values in the distance moved metric
    print(f"\nMissing values analysis:")
    print(
        f"  {distance_moved_col}: {test_ball_data[distance_moved_col].isna().sum()} missing out of {len(test_ball_data)}"
    )

    # Show actual values for debugging
    print(f"\nSample of actual values:")
    print(f"{distance_moved_col} (first 10 non-null values):")
    distance_sample = test_ball_data[distance_moved_col].dropna().head(10)
    print(distance_sample.tolist() if not distance_sample.empty else "No non-null values found")

    # Check unique values to see if there are unexpected values
    print(f"\nUnique values in {distance_moved_col} (first 20):")
    unique_distance_vals = test_ball_data[distance_moved_col].unique()
    print(f"Found {len(unique_distance_vals)} unique values: {unique_distance_vals[:20]}...")
    ...
    valid_data = test_ball_data[test_ball_data[distance_moved_col].notna()]
    ...
    mask = pd.isna(plot_data[distance_moved_col]) | np.isinf(plot_data[distance_moved_col])
    ...
    # Plot: distance_moved by Genotype + Pretraining
    create_boxplot_with_scatter_combined(
        data=plot_data_filtered,
        pretraining_col=pretraining_col,
        genotype_col=genotype_col,
        y_col=distance_moved_col,
        title="Distance Moved by Genotype + Pretraining",
        ax=ax,
    ...
    # Save the plot
    output_path = Path(__file__).parent / "distance_moved_boxplots.png"
    ...
    if distance_moved_col:
        print(f"\nDistance Moved by Genotype + Pretraining:")
        distance_data = plot_data_combined.dropna(subset=[distance_moved_col])
        if not distance_data.empty:
            distance_summary = (
                distance_data.groupby("combined_group")[distance_moved_col]
                .agg(["count", "mean", "std", "median", "min", "max"])
                .round(2)
            )
            print(distance_summary)
        else:
            print("No valid data for distance_moved")
    ...
    if distance_moved_col and len(unique_groups) >= 2:
        print(f"\nDistance Moved - Combined Group Analysis:")
        distance_data = plot_data_combined.dropna(subset=[distance_moved_col])

        if len(unique_groups) == 2:
            # Mann-Whitney U test for two groups
            group1_data = distance_data[distance_data["combined_group"] == unique_groups[0]][distance_moved_col]
            group2_data = distance_data[distance_data["combined_group"] == unique_groups[1]][distance_moved_col]
            ...
            # Kruskal-Wallis test for multiple groups
            group_data = [
                distance_data[distance_data["combined_group"] == group][distance_moved_col].values
                for group in unique_groups
                if len(distance_data[distance_data["combined_group"] == group]) > 0
            ]
            ...
            if len(group_data) > 1 and all(len(g) > 0 for g in group_data):
                ...
                for i, group in enumerate(unique_groups):
                    n = len(distance_data[distance_data["combined_group"] == group])
                    print(f"  {group}: n={n}")
