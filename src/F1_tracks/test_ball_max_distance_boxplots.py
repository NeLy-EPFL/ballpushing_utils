#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for test ball max_distance.
1. Max distance for test ball by Pretraining condition
2. Max distance for test ball by F1_Condition
"""
...
def create_boxplot_with_scatter(data, x_col, y_col, title, ax, color_mapping=None, order=None):
    ...
    ax.set_ylabel("Max Distance - Test Ball", fontsize=12)
...
def main():
    ...
    # Save the plot
    output_path = Path(__file__).parent / "test_ball_max_distance_boxplots.png"
    ...
    if pretraining_col and distance_col:
        print(f"\n1. Test Ball Max Distance by {pretraining_col}:")
        pretraining_summary = (
    ...
    if f1_condition_col and distance_col:
        print(f"\n2. Test Ball Max Distance by {f1_condition_col}:")
        f1_summary = (
