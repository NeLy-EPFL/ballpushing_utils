#!/usr/bin/env python3
"""
Script to create boxplots with superimposed scatter plots for final event metrics.
1. final_event by Pretraining condition
2. final_event_time by Pretraining condition
"""
...
def main():
    ...
    final_event_col = "final_event" if "final_event" in df.columns else None
    final_event_time_col = "final_event_time" if "final_event_time" in df.columns else None
    ...
    if "ball_condition" in df.columns:
        ...
        for col in df.columns:
            if "ball" in col.lower() and ("condition" in col.lower() or "identity" in col.lower()):
                ...
                break
    required_columns = {
        "final_event": final_event_col,
        "final_event_time": final_event_time_col,
    ...
    if missing:
        ...
        return
    df_clean = df[[final_event_col, final_event_time_col, pretraining_col, ball_condition_col]]
    ...
    event_data = test_ball_data.dropna(subset=[final_event_col])
    if not event_data.empty:
        create_boxplot_with_scatter(
            data=event_data,
            x_col=pretraining_col,
            y_col=final_event_col,
            title="Final Event by Pretraining Condition",
            ax=ax1,
            color_palette=pretraining_palette,
        )
    else:
        ax1.text(0.5, 0.5, "No valid data for final_event", ha="center", va="center", transform=ax1.transAxes)
        ax1.set_title("Final Event by Pretraining Condition")
    time_data = test_ball_data.copy()
    mask = pd.isna(time_data[final_event_time_col]) | np.isinf(time_data[final_event_time_col])
    time_data_filtered = time_data[~mask]
    if len(time_data_filtered) >= 2:
        create_boxplot_with_scatter(
            data=time_data_filtered,
            x_col=pretraining_col,
            y_col=final_event_time_col,
            title="Final Event Time by Pretraining Condition",
            ax=ax2,
            color_palette=pretraining_palette,
        )
    else:
        ...
        ax2.set_title("Final Event Time by Pretraining Condition")
        if pretraining_col:
            ax2.set_xlabel(pretraining_col.replace("_", " ").title())
        ax2.set_ylabel("Final Event Time")
    plt.tight_layout()
    output_path = Path(__file__).parent / "final_event_boxplots.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print(f"Plot saved to: {output_path}")
    print("\n" + "=" * 60)
    print("SUMMARY STATISTICS")
    print("=" * 60)
    if pretraining_col and final_event_col:
        print(f"\n1. Final Event by {pretraining_col}:")
        event_data = test_ball_data.dropna(subset=[final_event_col])
        if not event_data.empty:
            event_summary = (
                event_data.groupby(pretraining_col)[final_event_col]
                .agg(["count", "mean", "std", "median", "min", "max"])
                .round(3)
            )
            print(event_summary)
        else:
            print("No valid data for final_event")
    if pretraining_col and final_event_time_col:
        print(f"\n2. Final Event Time by {pretraining_col}:")
        time_data = test_ball_data.dropna(subset=[final_event_time_col])
        if not time_data.empty:
            time_summary = (
                time_data.groupby(pretraining_col)[final_event_time_col]
                .agg(["count", "mean", "std", "median", "min", "max"])
                .round(3)
            )
            print(time_summary)
        else:
            print("No valid data for final_event_time")
    correlation_data = test_ball_data.dropna(subset=[final_event_col, final_event_time_col])
    if not correlation_data.empty and len(correlation_data) > 1:
        correlation, p_value = stats.pearsonr(correlation_data[final_event_col], correlation_data[final_event_time_col])
        print(f"\nCorrelation between final event and time (n={len(correlation_data)}):")
