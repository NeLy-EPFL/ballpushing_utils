#!/usr/bin/env python3
"""
Script to generate Mann-Whitney U test jitterboxplots for broad TNT screen experiments.
...
Usage:
    python run_mannwhitney_broad_tnt.py [--no-overwrite] [--test]
...
def generate_text_report(stats_df, data, report_file):
    ...
    """
    report_lines = []
    report_lines.append("# Broad TNT Screen Genotype Mann-Whitney U Test Report")
...
def load_and_clean_dataset(test_mode=False, test_sample_size=200):
    ...
    """
    # Load the broad TNT screen dataset
    dataset_path = "/mnt/upramdya_data/MD/Broad_TNT/Datasets/251118_13_summary_broad_TNT_screen_Data/summary/pooled_summary.feather"

    print(f"Loading broad TNT screen dataset from: {dataset_path}")
    try:
...
def main(overwrite=True, test_mode=False):
    """Main function to run Mann-Whitney plots for broad TNT screen experiments.
    ...
    """
    print(f"Starting Mann-Whitney U test analysis for broad TNT screen experiments...")
    if not overwrite:
    ...
    base_output_dir = Path("/mnt/upramdya_data/MD/Broad_TNT/Plots/Summary_metrics/Genotype_Mannwhitney")
    ...
    print(f"\n{'='*60}")
    print("✅ Broad TNT screen genotype analysis complete! Check output directory for results.")
    print(f"📁 Output directory: {base_output_dir}")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate Mann-Whitney U test plots for broad TNT screen experiments",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_mannwhitney_broad_tnt.py                    # Overwrite existing plots
  python run_mannwhitney_broad_tnt.py --no-overwrite     # Skip existing plots
  python run_mannwhitney_broad_tnt.py --test             # Test mode with 3 metrics
        """,
    )
    parser.add_argument(
        "--no-overwrite",
