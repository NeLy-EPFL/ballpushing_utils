...
2. **Interaction duration** (mean seconds per event per 5-min bin)
...
Arguments:
    --bin-size         : Bin duration in minutes (default: 5)
...
def plot_boxplot_per_bin(
    ...
    for ax, mag in zip(axes, magnet_vals):
        ...
        ax.boxplot(
            ...
            positions=positions,
            patch_artist=True,
            boxprops=dict(facecolor=colors[mag], alpha=0.4),
            medianprops=dict(color="black", lw=1.5),
            whiskerprops=dict(color=colors[mag], alpha=0.7),
            capprops=dict(color=colors[mag], alpha=0.7),
...
def main():
    ...
    parser.add_argument("--bin-size", type=float, default=5.0, help="Time bin size in minutes (default: 5)")
