...
def create_ellipses_for_controls(pca_data, x_col, y_col, config):
    ...
    for control_name, color in CONTROL_ELLIPSES.items():
        ...
        if len(control_data) >= 3:
            ellipse_points = create_confidence_ellipse_data(control_data[x_col].values, control_data[y_col].values)
            ...
            if ellipse_points is not None:
                ...
                ellipse_plot = hv.Path(ellipse_df).opts(
                    color=color, alpha=ellipse_style["alpha"], line_width=ellipse_style["line_width"], tools=[]
                )
...
def create_scatter_plot(plot_data, x_col, y_col, x_label, y_label, config):
    ...
    for sig_type in ["significant", "non_significant"]:
...
def create_interactive_scatterplot_matrix(pca_data, static_results, temporal_results, config, n_components=6):
    ...
    # Create layout
    layout_opts = opts.Layout(shared_axes=config["layout"]["shared_axes"])
...
def main():
    ...
    parser.add_argument(
        "--formats", nargs="+", default=["html"], choices=["html"], help="Output formats (default: html only)"
    )
