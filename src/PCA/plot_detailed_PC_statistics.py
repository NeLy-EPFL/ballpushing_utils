...
def get_pc_direction_info(genotype, control, pca_method):
    """Get direction information (up/down regulation) for each PC"""

    # Load the PC projection data
    data_file = f"static_{pca_method}_with_metadata_tailoredctrls.feather"

    try:
        data = pd.read_feather(data_file)
    except FileNotFoundError:
        print(f"Warning: Could not find {data_file} for direction information")
        return {}

    # Get PC columns (they should be named PC1, PC2, etc. or SparsePCA1, etc.)
    method_prefix = "SparsePCA" if pca_method == "sparsepca" else "PC"
    pc_columns = [col for col in data.columns if col.startswith(method_prefix) and col[len(method_prefix) :].isdigit()]

    directions = {}

    for pc_col in pc_columns:
        # Get data for this genotype and control
        genotype_data = data[data["Nickname"] == genotype][pc_col]
        control_data = data[data["Nickname"] == control][pc_col]

        if len(genotype_data) > 0 and len(control_data) > 0:
            genotype_mean = genotype_data.mean()
            control_mean = control_data.mean()

            # Convert column name to standard PC format
            pc_name = pc_col.replace("SparsePCA", "PC").replace("PCA", "PC")

            # Direction: positive if genotype > control (upregulated)
            directions[pc_name] = 1 if genotype_mean > control_mean else -1
...
def perform_simplified_static_analysis():
    ...
    for _, row in permutation_significant.iterrows():
        ...
        # Get direction information for this genotype-control pair
        directions = get_pc_direction_info(nickname, control_name, pca_method)
        ...
        for i, pc in enumerate(available_components):
            # Convert SparsePCA component names to PC names
            pc_name = pc.replace("SparsePCA", "PC").replace("PCA", "PC")

            if i < len(mannwhitney_pvals) and i < len(mannwhitney_corrected):
                pc_results[f"{pc_name}_pval"] = mannwhitney_pvals[i]
                pc_results[f"{pc_name}_pval_corrected"] = mannwhitney_corrected[i]
                pc_results[f"{pc_name}_significant"] = pc in significant_dims
                pc_results[f"{pc_name}_direction"] = directions.get(pc_name, 0)
            else:
                pc_results[f"{pc_name}_pval"] = np.nan
                pc_results[f"{pc_name}_pval_corrected"] = np.nan
                pc_results[f"{pc_name}_significant"] = False
                pc_results[f"{pc_name}_direction"] = 0
    ...
    max_pcs_tested = 0
    if len(results_df) > 0:
        # Get the maximum number of tested PCs across all genotypes (using PC naming)
        pc_cols = [col for col in results_df.columns if col.startswith("PC") and col.endswith("_pval")]
        max_pcs_tested = len(pc_cols)
...
def create_hits_heatmap(results_df):
    """Create a detailed heatmap showing genotype PC significance with direction and p-values"""
    ...
    # Get all PC components that were tested (using PC naming convention)
    ...
    # Ensure we have all 10 PCs if they should be there
    expected_pcs = [f"PC{i}" for i in range(1, 11)]
    for pc in expected_pcs:
        if f"{pc}_significant" not in results_df.columns:
            # Add missing PC columns with False values
            results_df[f"{pc}_significant"] = False
            results_df[f"{pc}_pval_corrected"] = np.nan

    # Update pc_columns and pc_names to include all expected PCs
    pc_columns = [f"{pc}_significant" for pc in expected_pcs]
    pc_names = expected_pcs

    # Load comprehensive results to get direction information
    try:
        comprehensive_results = pd.read_csv(comprehensive_file)
    except FileNotFoundError:
        print(f"Warning: Could not load {comprehensive_file} for direction information")
    ...
    for _, row in significant_results.iterrows():
        ...
        for pc in expected_pcs:
            ...
            if is_significant and not pd.isna(pval_corrected):
                ...
                # Set color value based on direction
                sig_row.append(direction)  # 1 for up (red), -1 for down (blue)
                pval_row.append(pval_corrected)
    ...
    # Create DataFrames
    sig_df = pd.DataFrame(significance_matrix, index=genotype_names, columns=pc_names)
    annot_df = pd.DataFrame(annotation_matrix, index=genotype_names, columns=pc_names)

    # Set up the plot
    plt.style.use("default")
    fig, ax = plt.subplots(figsize=(14, max(8, len(genotype_names) * 0.4)))

    # Create custom colormap: blue for down (-1), white for no significance (0), red for up (1)
    ...
    sns.heatmap(
        sig_df,
        annot=annot_df,
        fmt="",  # Use string annotations
        cmap=cmap,
        center=0,
        vmin=-1,
        vmax=1,
        cbar=False,  # We'll add a custom legend instead
        ax=ax,
        linewidths=0.5,
        linecolor="gray",
    ...
    ax.set_title(
        f"Detailed PC Significance Heatmap - Static {method_display}\n"
        f"Mann-Whitney Test with FDR Correction\n"
    ...
    legend_elements = [
        ...
        Patch(facecolor="none", label=""),  # Spacer
