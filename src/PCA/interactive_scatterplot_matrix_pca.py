...
def prepare_plotting_data(pca_data, static_significance, temporal_significance, genotypes_with_samples):
    ...
    genotype_means = (
        ...
        .agg({col: "mean" for col in significant_data.columns if col.startswith("PCA")})
        .reset_index()
    )
...
def add_brain_region_legend_overlay(plot, genotype_means):
    ...
    # Title
    title_text = hv.Text(0.05, 0.98, "Brain Regions:", fontsize=10).opts(text_color="black", text_font_style="bold")
    ...
    for region in unique_regions[:6]:
        ...
        text = hv.Text(0.05, y_pos, f"● {region}", fontsize=9).opts(text_color=color)
        legend_elements.append(text)
        y_pos -= 0.05

    # Add "..." if there are more regions
    if len(unique_regions) > 6:
        more_text = hv.Text(0.05, y_pos, "...", fontsize=9).opts(text_color="gray")
        legend_elements.append(more_text)

    # Create overlay with legend
    legend_overlay = hv.Overlay(legend_elements).opts(xaxis=None, yaxis=None, show_grid=False, toolbar=None)
...
def add_significance_legend_overlay(plot, genotype_means):
    ...
    # Title
    title_text = hv.Text(0.05, 0.98, "Marker Shapes:", fontsize=10).opts(text_color="black", text_font_style="bold")
    legend_elements.append(title_text)
    ...
    for sig_type in sorted(sig_types):
        ...
        text = hv.Text(0.05, y_pos, f"{marker_symbol} {sig_type.split(' ')[0]}", fontsize=9).opts(text_color="black")
        legend_elements.append(text)
...
def add_ellipses_legend_overlay(plot):
    ...
    # Title
    title_text = hv.Text(0.05, 0.98, "Control Ellipses:", fontsize=10).opts(text_color="black", text_font_style="bold")
    legend_elements.append(title_text)

    # Control line descriptions
    controls = [("Empty-Split", "#ffb7b7"), ("Empty-Gal4", "#C6F7B0"), ("TNTxPR", "#B4EAFF")]

    y_pos = 0.93
    for control_name, color in controls:
        # Create dashed line representation
        line_text = hv.Text(0.05, y_pos, f"- - - {control_name}", fontsize=9).opts(text_color=color)
...
def create_comprehensive_legend(genotype_means, all_significant, static_significant, temporal_significant):
    ...
    for i, region in enumerate(unique_regions):
        brain_data.append({"x": i * 0.8, "y": 1, "region": region, "color": brain_region_colors.get(region, "#000000")})

    brain_df = pd.DataFrame(brain_data)
    brain_legend = hv.Scatter(brain_df, kdims="x", vdims=["y", "region", "color"]).opts(
        color="color",
        size=8,
        alpha=0.8,
        ylabel="",
    ...
    if sig_data:
        ...
        for item in sig_data:
            scatter = hv.Scatter(pd.DataFrame([item]), kdims="x", vdims=["y", "type", "marker", "size"]).opts(
                color="gray",
                size="size",
                marker="marker",
                alpha=0.8,
    ...
    for item in control_data:
        line = hv.Curve(
            pd.DataFrame({"x": item["x"], "y": item["y"]}), kdims="x", vdims="y", label=item["control"]
        ).opts(
            color=item["color"],
            line_width=3,
            line_dash="dashed",
...
def create_external_legends(genotype_means, all_significant, static_significant, temporal_significant):
    ...
    brain_legend = hv.Scatter(brain_legend_df, kdims="x", vdims=["y", "region", "color"], label="Brain Regions").opts(
        color="color",
        size=8,
        alpha=0.8,
        width=150,
    ...
    if sig_legend_data:
        ...
        for _, row in sig_legend_df.iterrows():
            scatter = hv.Scatter(pd.DataFrame([row]), kdims="x", vdims=["y", "type", "marker", "size"]).opts(
                color="gray",
                size="size",
                marker="marker",
                alpha=0.8,
...
def create_standalone_legend(genotype_means, all_significant, static_significant, temporal_significant):
    ...
    brain_text_elements = []
    for i, region in enumerate(unique_regions):
        text = hv.Text(0.6, len(unique_regions) - i - 1, region, fontsize=12).opts(text_color="black")
    ...
    for item in control_data:
        ...
        line_data = pd.DataFrame({"x": [0.3, 0.7], "y": [item["y"], item["y"]]})
        ...
        # Add text label
        text = hv.Text(0.75, item["y"], item["name"], fontsize=12).opts(text_color="black")
    ...
    combined_legend = (
        hv.Layout([brain_legend_with_text, sig_legend, control_legend])
        .cols(3)
        .opts(title="PCA Scatterplot Matrix Legend")
    )
