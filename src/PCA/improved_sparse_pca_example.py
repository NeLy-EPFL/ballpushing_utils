...
def plot_sparsity_analysis(results, feature_names):
    ...
    for method_name, result in results.items():
        if "alpha" in method_name or method_name == "regular_pca":
            axes[0, 1].plot(
                range(1, len(result["explained_variance_ratio"]) + 1),
    ...
    for method_name, result in results.items():
        if "alpha" in method_name or method_name == "regular_pca":
            axes[1, 0].plot(
                range(1, len(result["n_nonzero_per_component"]) + 1),
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/matthias/ballpushing_utils/outputs/pca_outputs/sparse_pca_analysis.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
...
def create_statistical_comparison_data(regular_pca_result, sparse_pca_result, metadata):
    ...
    regular_pca_df = pd.DataFrame(
        regular_pca_result["scores"], columns=[f"PCA{i+1}" for i in range(regular_pca_result["scores"].shape[1])]
    )

    sparse_pca_df = pd.DataFrame(
        sparse_pca_result["scores"], columns=[f"SparsePCA{i+1}" for i in range(sparse_pca_result["scores"].shape[1])]
    )
