...
def _parse_args():
    ...
    parser.add_argument(
        "--all-lines-heatmap-only",
        action="store_true",
        default=False,
        help="Skip all other steps and only generate the brain-region grouped heatmap for ALL lines in the screen. "
             "If pre-computed metric results exist in OUTPUT_DIR they will be reused to avoid rerunning permutation tests.",
    )
...
def plot_brain_region_grouped_heatmap(
    ...
    gs = gridspec.GridSpec(
        ...
        hspace=0.08,  # Tight spacing between brain regions
        wspace=0.05,
    )
    ...
    for region_idx, region in enumerate(sorted_regions):
        ...
        # Add region title above the heatmap using a fixed-point offset so it
        # never overlaps regardless of panel height (transAxes fractions would
        # scale with panel size and overshoot for tall panels like LH).
        from matplotlib.transforms import ScaledTranslation
        region_color = color_dict.get(region, "black")
        region_display_name = get_brain_region_display_name(region)
        fixed_offset = ScaledTranslation(0, 4 / 72, fig.dpi_scale_trans)
        ax.text(
            0.0,
            1.0,
            region_display_name,
            ...
            ha="left",
            va="bottom",
            transform=ax.transAxes + fixed_offset,
            clip_on=False,
        )
    ...
    # Use axes-coordinate x-positions (data coords mapped to axes) for rock-solid placement.
    if axes and valid_groups:
        ...
        group_colors = ["#222222", "#777777"]  # alternating dark-gray / mid-gray
        line_lw = 2.5
        ...
        ax_to_fig = ax_bottom.transAxes + fig.transFigure.inverted()

        # Find the lowest y-point of any x-tick label in figure coords
        y_min_fig = float("inf")
        for lbl in ax_bottom.get_xticklabels():
            ...
            y_min_fig = min(y_min_fig, pts[:, 1].min())
        if not np.isfinite(y_min_fig):
            y_min_fig = ax_to_fig.transform([0, 0])[1]

        y_line_fig = y_min_fig - 0.006
        y_label_fig = y_line_fig - 0.014

        for g_idx, (group_label, group_keys) in enumerate(valid_groups):
            present = [k for k in group_keys if k in col_idx_map]
            if not present:
                continue
            j0 = col_idx_map[present[0]]
            j1 = col_idx_map[present[-1]]
            color = group_colors[g_idx % len(group_colors)]

            # Convert data x-positions [j0+0.1, j1+0.9] to figure coords
            x0_ax = (j0 + 0.05) / n_cols
            x1_ax = (j1 + 0.95) / n_cols
            x0_fig = ax_to_fig.transform([x0_ax, 0])[0]
            x1_fig = ax_to_fig.transform([x1_ax, 0])[0]
    ...
    # Save
    base = f"metric_brain_region_grouped_heatmap{output_suffix}"
    png = os.path.join(OUTPUT_DIR, f"{base}.png")
    pdf = os.path.join(OUTPUT_DIR, f"{base}.pdf")
    svg = os.path.join(OUTPUT_DIR, f"{base}.svg")
    eps = os.path.join(OUTPUT_DIR, f"{base}.eps")
...
def main():
    ...
    # Fast path: skip everything except the all-lines brain-region heatmap
    if args.all_lines_heatmap_only:
        print("🚀 ALL-LINES HEATMAP ONLY MODE")
        print("=" * 50)
        ...
        # Reuse pre-computed results if available (avoids re-running slow permutation tests)
        test_method = "permutation" if USE_PERMUTATION_PER_PC else "mannwhitney"
        precomputed_path = os.path.join(OUTPUT_DIR, f"metric_stats_results_{test_method}_all_lines.csv")
        fallback_path = os.path.join(OUTPUT_DIR, f"metric_stats_results_{test_method}.csv")
        correlation_matrix_path = os.path.join(OUTPUT_DIR, "metric_correlations.csv")

        if os.path.exists(precomputed_path):
            print(f"✅ Loading pre-computed all-lines results: {precomputed_path}")
            results_df_all = pd.read_csv(precomputed_path)
            correlation_matrix_all = pd.read_csv(correlation_matrix_path, index_col=0) if os.path.exists(correlation_matrix_path) else None
        else:
            print("🔬 No pre-computed all-lines results found — running metric analysis (this will take a while)...")
            all_genotypes = sorted(dataset["Nickname"].dropna().unique().tolist())
            print(f"   📊 Analyzing {len(all_genotypes)} lines...")
            results_df_all, correlation_matrix_all = run_metric_analysis(dataset, metrics_list, all_genotypes)
            if not results_df_all.empty:
                # Save with dedicated suffix for reuse
                results_df_all.to_csv(precomputed_path, index=False)
                print(f"   💾 Saved all-lines results to {precomputed_path}")

        if results_df_all is not None and not results_df_all.empty and correlation_matrix_all is not None:
            n_lines = len(results_df_all)
            print(f"\n🎨 Creating brain region grouped heatmap for {n_lines} lines...")
            plot_brain_region_grouped_heatmap(
                ...
                only_significant_hits=False,
                alpha=0.05,
                fig_size=(20, max(14, n_lines * 0.25)),
                output_suffix="_all_lines",
            )
            print(f"\n✅ All-lines heatmap saved to {OUTPUT_DIR}")
        else:
    ...
    # Create brain region grouped heatmap for ALL lines in the screen
    print(f"\n🎨 Creating brain region grouped heatmap for ALL screen lines...")
    all_genotypes = sorted(dataset["Nickname"].dropna().unique().tolist())
    print(f"   📊 Running metric analysis for all {len(all_genotypes)} lines...")
    results_df_all, correlation_matrix_all = run_metric_analysis(dataset, metrics_list, all_genotypes)
    if not results_df_all.empty:
        plot_brain_region_grouped_heatmap(
            results_df_all,
            ...
            only_significant_hits=False,
            alpha=0.05,
            fig_size=(20, max(14, len(all_genotypes) * 0.25)),
            output_suffix="_all_lines",
        )
    else:
        print("   ⚠️  No results for all-lines heatmap.")
