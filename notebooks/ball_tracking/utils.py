...
def concat2d(im_list, n_rows, n_cols, fill_value=0, dtype=np.uint8):
    ...
    im = np.full(
        (n_rows * n_cols, *im_list[0].shape), fill_value=fill_value, dtype=dtype
    )
...
def uniform_sample_frames(
    ...
    try:
        ...
        ), f"Requested {n_frames} frames, but video has only {n_frames_total} frames."
        ...
        frames = np.ascontiguousarray(
            frames.reshape((t, n_corridors, corridor_height, w))
        )
        return frames
    except ImportError:
        import warnings
        ...
        warnings.warn(
            "video_reader is not installed (install with: pip install 'ballpushing_utils[video]'). "
        ...
        frame_indices = np.linspace(
            0, n_frames_total - 1, n_frames, endpoint=False
        ).astype(int)
        width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        frames = np.zeros(
            (n_frames, n_corridors, corridor_height, width), dtype=np.uint8
        )
        for j, i in enumerate(frame_indices):
            ...
            if not ret:
                break
            frames[j] = frame[y0:y1, :, 0].reshape(
                (n_corridors, corridor_height, width)
            )
        cap.release()
        return frames
