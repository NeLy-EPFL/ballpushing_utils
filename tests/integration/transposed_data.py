...
def process_fly(fly_path):
    ...
    transposedData = Dataset(ExampleFly, dataset_type="transposed")
...
if __name__ == "__main__":
    ...
    if args.mode == "fly":
        transposed_dataset = process_fly(Path(args.path))

    elif args.mode == "experiment":
        transposed_dataset = process_experiment(Path(args.path))

    # Save the transposed dataset in a csv file
    if transposed_dataset.data is not None:
        output_dir = Path(__file__).parent / "outputs"
        output_dir.mkdir(exist_ok=True)  # Ensure the 'outputs' directory exists
        output_path = output_dir / "transposed_dataset_columns.csv"
        transposed_dataset.data.to_csv(output_path, index=False)
