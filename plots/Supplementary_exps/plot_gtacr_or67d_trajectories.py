#!/usr/bin/env python3
"""
Script to generate ball trajectory plots over time for broad TNT screen experiments.
...
Usage:
    python plot_broad_tnt_trajectories.py [--n-bins N] [--n-permutations N] [--output-dir PATH]
...
Arguments:
    ...
    --output-dir: Directory to save plots (default: /mnt/upramdya_data/MD/Broad_TNT/Plots/trajectories)
"""
...
def load_coordinates_dataset():
    """Load the broad TNT screen coordinates dataset with genotype harmonization and ATR handling"""
    dataset_path = (
        "/mnt/upramdya_data/MD/Gtacr/Datasets/251219_10_summary_OR67d_Gtacr_Data/coordinates/pooled_coordinates.feather"
    )
    ...
    if "Date" in dataset.columns:
        ...
        print(
            f"Dataset shape after filtering Date == 230719: {dataset.shape} (removed {original_shape[0] - dataset.shape[0]} rows)"
        )
    else:
        print("Warning: 'Date' column not found in dataset, skipping date filter")
...
def generate_all_trajectory_plots(
    ...
    atr_condition : str, optional
        ATR condition to filter by (e.g., "yes", "no")
    """
    if output_dir is None:
        output_dir = Path("/mnt/upramdya_data/MD/Gtacr/Plots/trajectories")
    else:
    ...
    print(f"\n{'='*60}")
    print("GENERATING BROAD TNT SCREEN TRAJECTORY PLOTS")
    if atr_condition:
        print(f"ATR CONDITION: {atr_condition}")
    print(f"{'='*60}")
    print(f"Output directory: {output_dir}")
    print(f"Time bins: {n_bins}")
    print(f"Permutations: {n_permutations}")

    # Filter by ATR condition if specified
    if atr_condition and "ATR" in data.columns:
        original_len = len(data)
        data = data[data["ATR"] == atr_condition].copy()
        print(f"Filtered to ATR={atr_condition}: {len(data)} rows (from {original_len})")
    ...
    with open(stats_file, "w") as f:
        f.write("Broad TNT Screen Trajectory Permutation Test Results\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate ball trajectory plots for broad TNT screen experiments",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python plot_broad_tnt_trajectories.py
  python plot_broad_tnt_trajectories.py --n-bins 15 --n-permutations 5000
  python plot_broad_tnt_trajectories.py --output-dir /path/to/output
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to save plots (default: auto-detect based on ATR structure)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="GtacrxOR67d",
        help="Control genotype name (default: GtacrxOR67d)",
    )
    ...
    # Load data
    print("Loading broad TNT screen coordinates data...")
    data = load_coordinates_dataset()

    # Check for ATR conditions
    atr_conditions = sorted(data["ATR"].unique()) if "ATR" in data.columns else ["all"]
    ...
    for atr_condition in atr_conditions:
        ...
        # Determine output directory for this ATR condition
        if args.output_dir:
            output_dir = Path(args.output_dir) / f"ATR_{atr_condition}"
        else:
            base_output = Path("/mnt/upramdya_data/MD/Gtacr/Plots/summaries/Genotype_Mannwhitney")
            output_dir = base_output / f"ATR_{atr_condition}" / "trajectories"

        print(f"Output directory: {output_dir}")

        # Generate plots for this ATR condition
        generate_all_trajectory_plots(
            ...
            atr_condition=atr_condition if atr_condition != "all" else None,
        )

        print(f"\n✅ ATR={atr_condition} trajectories complete")
        print(f"📁 Saved to: {output_dir}")

    print(f"\n{'='*80}")
    print("✅ ALL TRAJECTORY PLOTS COMPLETE")
    print(f"{'='*80}")
    print(f"Processed {len(atr_conditions)} ATR condition(s)")
