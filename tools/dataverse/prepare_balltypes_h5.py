#!/usr/bin/env python3
"""
Prepare ball-type raw H5 files for Dataverse upload.

Layout:
  <OUTPUT_ROOT>/<BallType>/<Date>/<Arena>/<Corridor>/<h5 files>

Each experiment folder in ball_types.yaml contains:
  - metadata.json  (Arena# entries include BallType and Date)
  - arena{1..9}/corridor{1..6}/*tracked*.h5 and *full_body.h5

Usage
-----
  # Dry-run for a single experiment (index 0 in YAML):
  python prepare_balltypes_h5.py --dry-run --experiment-index 0

  # Run all pre-flight checks without copying:
  python prepare_balltypes_h5.py --validate

  # Full copy (all experiments):
  python prepare_balltypes_h5.py --copy

  # Full copy + archive each ball-type folder to .tar:
  python prepare_balltypes_h5.py --copy --compress
"""
...
YAML_PATH = Path(__file__).resolve().parents[2] / "experiments_yaml/ball_types.yaml"
OUTPUT_ROOT = dataset("Affordance_article_dataverse/ball_types/raw_h5")
N_ARENAS = 9
N_CORRIDORS = 6
MAX_ARCHIVE_SIZE_BYTES = 2 * 1024**3

# Metadata -> directory labels.
# Includes typo handling: "sillicon" is normalized to "Silicone".
BALLTYPE_LABELS: dict[str, str | None] = {
    "ctrl": "Balltype-Steel",
    "manufactured": "Balltype-Manufactured",
    "rusty": "Balltype-Rusty",
    "sand": "Balltype-Sandpaper",
    "sillicon": "Balltype-Silicone",
    "silicon": "Balltype-Silicone",
    "": None,
}

EXPECTED_BALLTYPE_DIRS: set[str] = {
    "Balltype-Steel",
    "Balltype-Manufactured",
    "Balltype-Rusty",
    "Balltype-Sandpaper",
    "Balltype-Silicone",
}
...
def balltype_dir(balltype_raw: str) -> str | None:
    """Return normalized ball-type directory name or None to skip."""
    return BALLTYPE_LABELS.get(balltype_raw)
...
def _collect_slots(
    ...
    """Return {(balltype, date, arena_idx): [exp_dirs]} for collision detection."""
    ...
    for exp_dir in sorted(experiments):
        ...
        for arena_idx in range(1, N_ARENAS + 1):
            ...
            if arena_key not in meta:
                continue
            try:
                balltype_raw = metadata_get(meta, arena_key, "BallType")
                date = metadata_get(meta, arena_key, "Date")
            except (KeyError, IndexError):
                continue
            balltype = balltype_dir(balltype_raw)
            if balltype is None or not date or date == "None":
                continue
            slot_exps[(balltype, date, arena_idx)].append(exp_dir)
    return slot_exps
...
def plan_experiment(exp_dir: Path, output_root: Path) -> list[tuple[Path, Path]]:
    ...
    for arena_idx in range(1, N_ARENAS + 1):
        ...
        balltype_raw = metadata_get(metadata, arena_key, "BallType")
...
def plan_all_experiments(
    ...
    Handles same-day same-balltype same-arena collisions by appending a numeric
    suffix to date (e.g. 240411-1, 240411-2).
    """
    ...
    for (balltype, date, arena_idx), exps in slot_exps.items():
        slot = (balltype, date, arena_idx)
        if len(exps) == 1:
            ...
            for i, exp_dir in enumerate(exps, 1):
                ...
                print(
                    f"  [INFO] Date collision: {balltype}/Arena{arena_idx} on {date} "
...
def validate(
    ...
    print("\n[Test 1] Checking BallType values and metadata availability...")
    missing_exps: list[Path] = []
    unknown_values: set[str] = set()
    balltypes_seen: set[str] = set()
    ...
    for exp_dir in experiments:
        ...
        for arena_idx in range(1, N_ARENAS + 1):
            ...
            try:
                ...
                msg = f"  [ERROR] 'BallType' missing in metadata for {exp_dir}"
                print(msg)
                issues.append(msg)
                break
            if balltype_raw not in BALLTYPE_LABELS:
                unknown_values.add(balltype_raw)
            else:
                mapped = balltype_dir(balltype_raw)
                if mapped:
                    balltypes_seen.add(mapped)
    ...
    if unknown_values:
        for v in sorted(unknown_values):
            print(f"  [WARN] Unknown BallType value: {v!r} - arena will be skipped")
    else:
        print("  OK - all BallType values are recognized")

    missing_dirs = EXPECTED_BALLTYPE_DIRS - balltypes_seen
    if missing_dirs:
        for d in sorted(missing_dirs):
            print(f"  [WARN] BallType not present in this dataset: {d}")
    ...
    print("\n[Info] Files per ball type:")
    bt_counts: dict[str, int] = defaultdict(int)
    for _, dst in all_pairs:
        bt_name = dst.relative_to(output_root).parts[0]
        bt_counts[bt_name] += 1
    for bt_name, count in sorted(bt_counts.items()):
        print(f"  {bt_name}: {count} files")
...
def dry_run(exp_dir: Path, experiments: list[Path], output_root: Path) -> None:
    ...
    for arena_idx in range(1, N_ARENAS + 1):
        ...
        print(f"  Arena{arena_idx}: BallType={balltype_raw!r} -> {label} (date={date})")
...
def compress_balltypes(output_root: Path) -> None:
    """Archive each top-level ball-type folder into one or more .tar files."""
    bt_dirs = sorted(
        [d for d in output_root.iterdir() if d.is_dir()], key=lambda p: p.name
    )
    print(f"\nArchiving {len(bt_dirs)} ball-type folders...")
    for bdir in bt_dirs:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare ball-type H5 files for Dataverse upload",
    ...
    parser.add_argument(
        "--compress",
        action="store_true",
        help="After copying, archive each ball-type folder to .tar (used with --copy)",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def main() -> None:
    ...
    if args.compress:
        compress_balltypes(args.output_root)
