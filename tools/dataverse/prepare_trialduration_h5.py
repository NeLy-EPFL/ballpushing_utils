#!/usr/bin/env python3
"""
Prepare trial-duration raw H5 files for Dataverse upload.

Layout:
    <OUTPUT_ROOT>/Multi-trials/<Date>/<Arena>/<Corridor>/<h5 files>

Each experiment folder in ball_replacement_folders.yaml contains:
  - metadata.json
...
-----
  ...
  python prepare_trialduration_h5.py --dry-run --experiment-index 0

  # Run all pre-flight checks without copying:
  python prepare_trialduration_h5.py --validate

  # Full copy (all experiments):
  python prepare_trialduration_h5.py --copy

    # Full copy + archive one Multi-trials.tar containing all dates:
  python prepare_trialduration_h5.py --copy --compress
"""
...
YAML_PATH = Path(__file__).resolve().parents[2] / "experiments_yaml/ball_replacement_folders.yaml"
OUTPUT_ROOT = dataset("Affordance_article_dataverse/trial_duration/raw_h5")
N_ARENAS = 9
N_CORRIDORS = 6
TRIALS_DIR = "Multi-trials"
...
def _collect_slots(
    ...
    """Return {(date, arena_idx): [exp_dirs]} for date-collision detection."""
    ...
    for exp_dir in sorted(experiments):
        ...
        for arena_idx in range(1, N_ARENAS + 1):
            ...
            if not date or date == "None":
                continue
            slot_exps[(date, arena_idx)].append(exp_dir)
    return slot_exps
...
def plan_all_experiments(
    ...
    Handles same-day same-arena collisions by appending a numeric suffix to date
    (e.g. 240723-1, 240723-2).
    """
    ...
    for (date, arena_idx), exps in slot_exps.items():
        slot = (date, arena_idx)
        if len(exps) == 1:
            ...
            for i, exp_dir in enumerate(exps, 1):
                ...
                print(
                    f"  [INFO] Date collision: Arena{arena_idx} on {date} "
...
def validate(
    ...
    print("\n[Test 1] Checking metadata/date availability...")
    ...
    for exp_dir in experiments:
        ...
        for arena_idx in range(1, N_ARENAS + 1):
            ...
            try:
                ...
                msg = f"  [ERROR] 'Date' variable missing in {exp_dir}"
                print(msg)
                issues.append(msg)
                break
            if not date or date == "None":
                missing_date.append(f"{exp_dir.name}/{arena_key}")
    if missing_exps:
        for p in missing_exps:
            ...
            issues.append(msg)
    if missing_date:
        for item in missing_date[:10]:
            print(f"  [WARN] Missing date: {item}")
        if len(missing_date) > 10:
            print(f"  [WARN] ... and {len(missing_date) - 10} more")
    if not missing_exps and not any("'Date' variable missing" in i for i in issues):
        print("  OK — metadata/date is available")
    ...
    for _, dst in all_pairs:
        ...
        if len(rel_parts) < 2 or rel_parts[0] != TRIALS_DIR:
            continue
        date_name = rel_parts[1]
        date_counts[date_name] += 1
    for date_name, count in sorted(date_counts.items()):
        print(f"  {date_name}: {count} files")
...
def compress_trials(output_root: Path) -> None:
    """Archive Multi-trials into one or more .tar files, then remove source."""
    trials_dir = output_root / TRIALS_DIR
    if not trials_dir.exists() or not trials_dir.is_dir():
        print(f"\n[SKIP] {trials_dir} does not exist")
        return

    print(f"\nArchiving {trials_dir.name} ...")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare trial-duration H5 files for Dataverse upload",
    ...
    parser.add_argument(
        "--compress",
        action="store_true",
        help="After copying, archive Multi-trials folder to Multi-trials.tar (used with --copy)",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def main() -> None:
    ...
    if args.compress:
        compress_trials(args.output_root)
